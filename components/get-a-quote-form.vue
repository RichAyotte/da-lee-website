...
<div class="container">
	...
	>
		...
		<div class="form-group row">
			<div class="col-sm-6">
				<label for="requestOptions" class="col-form-label">Request</label>
			...
			<div class="col-sm-6">
				<label for="regardingOptions" class="col-form-label">Regarding</label>
				<b-form-checkbox-group
					:options="regardingOptions"
					:state="!$v.checkedRegardingOptions.$dirty ? null : !$v.checkedRegardingOptions.$invalid"
					id="regardingOptions"
					name="regardingOptions"
					required
					stacked
					v-model="checkedRegardingOptions"
				/>
			</div>
		</div>
		<div class="form-group row">
...
const defaults = {
	...
	, regardingOptions() {
		return [
			{
				text: 'Dust Control & Winter Services'
				, value: 'Dust Control & Winter Services'
			}
			, {
				text: 'Pavement Products'
				, value: 'Pavement Products'
			}
			, {
...
export default {
	...
	, data() {
		return {
			...
			, message: ''
			, phone: ''
			, regardingOptions: defaults.regardingOptions()
			, recaptchaToken: ''
	...
	, methods: {
		reset() {
			...
			this.message = ''
			this.phone = ''
			this.regardingOptions = defaults.regardingOptions()
		...
		, async submit() {
			...
			const response = await axios.post(`/get-a-quote.php`, this.$data)
